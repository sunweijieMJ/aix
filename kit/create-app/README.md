...
缓存根目录默认 `~/.cache/create-app/`，设了 `XDG_CACHE_HOME` 则落在 `$XDG_CACHE_HOME/create-app/`。

`--force` 只管目标目录，与缓存无关 —— 所以 `--force --offline`（清空目录 + 只用本地缓存）
是合法组合。默认复用缓存时 CLI 会打印缓存年龄（`复用模板缓存（3 天前拉取）…`），
「模板改了怎么没生效」多半就是这里。复用缓存还会用 `git ls-remote` 探一次远端
（5 秒超时，失败静默跳过），远端确实前进了就直接警告
（`模板远端已有新提交（本地 abc1234 → 远端 def5678），如需最新加 --refresh`）。
克隆时记录的 commit 落在缓存目录的兄弟文件 `<缓存目录>.meta.json` 里 —— 放目录内会被
一起拷进新项目。
...
| 方式                                    | 组合数         | 用在                                            |
| --------------------------------------- | -------------- | ----------------------------------------------- |
| `--features a,b`（可重复，`''` = 全关） | 你给几个算几个 | 只想验某几种形态                                |
| `--combo <预设>`                        | 1              | 内置预设（admin 的特性 id）                     |
| `--smart-combos`                        | N + 2          | **推荐**：全开 + 全关 + 每个特性单独关一次      |
| `--all-combos`                          | 2^N            | 穷尽；配 `--install` 会非常慢（会打印预估耗时） |

`--smart-combos` / `--all-combos` 的特性 id 从**模板自己的 `.template/config.ts`** 枚举
（复用 `TemplateResolver`，含 Zod 校验），所以对任何模板都成立，不是写死 admin 的那几个。
