...
// ==================== 32. 模板字符串内含 HTML 标签（负向：整体跳过并告警）====================
// ❌ 不应提取：含 HTML 标签的模板串会被「整体跳过」（common-ast-utils 的 HTML 检测），
// 避免把 <div>/<span> 等结构塞进 locale 值（翻译/还原都无法保证标签完整）。
// 预期：generate 阶段对以下两条各产生一条 "HTML in template literal" 警告，
//      且其中的中文（"未读消息…条" / "请阅读…内容"）不出现在任何 locale 产物中。
// 与单测 html-template-skip.test.ts 形成 e2e 对照。
export const renderUnreadBadge = (count: number): string => {
  return `<span class="badge">未读消息 ${count} 条，请及时查看</span>`;
};

export const renderInfoCard = (title: string): string => {
  return `<div class="card"><h3>${title}</h3><p>请阅读以下重要说明内容</p></div>`;
};

// ==================== 33. 复杂表达式占位符的往返（A1：member / index / call / chain）====================
// ✅ 应提取：模板串里的「非字面量插值」整体作为占位符 {name}，name 由
// getVariableNameFromExpression 从表达式末段推导（member→属性名、call→函数名…）。
// Why：git 历史修过「复杂表达式占位符丢失」，本 section 作为回归守门——
// extract→generate→restore 后这些 ${...} 必须原样还原，且 locale 值里是 {占位符} 而非中文。
export const formatComplexPlaceholders = (
  order: { price: { final: number }; buyer: { name: string } },
  items: Array<{ qty: number }>,
): string[] => {
  return [
    `买家${order.buyer.name}的订单已创建`, // 成员访问 order.buyer.name
    `共${items[0]?.qty ?? 0}件商品待发货`, // 数组下标 + 可选链 items[0]?.qty
    `应付金额￥${order.price.final.toFixed(2)}元`, // 方法链 .toFixed(2)
    `单笔最多${Math.max(...items.map((it) => it.qty))}件`, // 调用表达式 Math.max(...)
  ];
};

// ==================== 34. 逻辑运算插值中的嵌套中文（B1：&& / || → 诊断）====================
// ⚠️ 不自动提取、应记入 nested-interpolation-chinese 诊断：与三元同理，
// `${cond && '中文'}` / `${x || '默认值'}` 的展示分支是渲染给用户的文案，却作为运行时
// 值塞入占位符——切语种后泄漏未翻译中文。collectNestedChineseLiterals 整树 walk，
// 逻辑运算与三元走同一收集路径（common-ast-utils:144-157）。
export const logicalInterpolation = (saving: boolean, name: string): string => {
  return `状态：${saving && '保存中，请稍候'}；当前用户：${name || '匿名访客'}`;
};

// ==================== 35. 数字 / 布尔 / null 字面量插值内联（B2）====================
// ✅ 应提取且内联：字面量插值（NumericLiteral / true / false / null）会被内联进
// processedText（common-ast-utils:100-115），locale 值是「展开后的常量」而非占位符，
// 保证占位符计数与运行时渲染一致。预期 locale 值含字面 3 / true / null。
export const literalInterpolation = (): string => {
  return `默认显示 ${3} 条，启用状态 ${true}，空值占位 ${null}`;
};

// ==================== 36. 低信息量占位符的兜底命名（B3：→ {value} 自动编号）====================
// ✅ 应提取：单字符 / 下划线前缀变量信息量低，getVariableNameFromExpression 兜底为
// {value} 并自动编号（{value}/{value1}/…），避免 locale 里出现无意义的 {i}{j}。
// Why：占位符名若不稳定，restore 阶段无法把 {i} 映射回 ${i}，导致占位符错配。
export const lowSignalPlaceholders = (i: number, j: number, _x: string): string => {
  return `第 ${i} 行第 ${j} 列的取值为 ${_x}`;
};
