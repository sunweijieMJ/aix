...
Aix 提供了强大而灵活的国际化系统，基于 Vue 3 的响应式特性和浏览器原生 Intl API 实现。
...
const { t, locale, formatters } = useLocale(myLocale);
...
// 使用格式化器
formatters.date.format(new Date(), 'short');  // '2025/10/17'
```
...
基于浏览器原生 `Intl` API，提供高性能的国际化格式化。

### DateFormatter - 日期格式化

```typescript
const { formatters } = useLocale(myLocale);

// 短日期: 2025/10/17
formatters.date.format(new Date(), 'short');

// 长日期: 2025年10月17日
formatters.date.format(new Date(), 'long');

// 时间: 下午2:30:00
formatters.date.format(new Date(), 'time');

// 相对时间: 3天前
formatters.date.relative(date, baseDate);
```
...
interface DateFormatter {
  format(date: Date, style: 'short' | 'long' | 'time'): string;
  relative(date: Date, baseDate?: Date): string;
}
```
...
```typescript
// 小数: 1,234.56
formatters.number.format(1234.56);

// 百分比: 75%
formatters.number.format(0.75, { style: 'percent' });

// 紧凑格式: 1.2K
formatters.number.format(1234, { notation: 'compact' });
...
```typescript
// 人民币: ¥1,234.56
formatters.currency.format(1234.56, 'CNY');

// 美元: $1,234.56
formatters.currency.format(1234.56, 'USD');

// 欧元: €1,234.56
formatters.currency.format(1234.56, 'EUR');

// 日元: ¥1,235 (无小数)
formatters.currency.format(1234.56, 'JPY');
```
...
### PluralFormatter - 复数处理

```typescript
// 英文: 1 item
formatters.plural.format(1, {
  one: '{count} item',
  other: '{count} items',
});

// 英文: 5 items
formatters.plural.format(5, {
  one: '{count} item',
  other: '{count} items',
});
...
function getFormatters(locale: Locale): Formatters {
  if (!formatterCache.has(locale)) {
    formatterCache.set(locale, createFormatters(locale));
  }
...
```typescript
// ✅ 推荐：使用格式化器
formatters.date.format(date, 'short');
...
**原因**：浏览器不支持该语言的 Intl API

**解决**：
1. 使用 Polyfill: `@formatjs/intl-pluralrules`
2. 或检查浏览器支持情况
