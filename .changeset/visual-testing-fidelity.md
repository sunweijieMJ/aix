...
新增 `visual-test fidelity` 设计还原度校验：Figma 节点树归一化为 DesignSpec，Playwright 渲染后提取 DOM 的 RenderSpec，按 `data-figma` → 文本 → 几何 IoU 三级匹配后做确定性的属性级 diff，输出面向 agent 的 `fidelity.md` / `fidelity.json`。
