...
import type { InstallConfig, InstallResult, Phase } from '../types/index.js';
...
export async function install(config: InstallConfig): Promise<InstallResult> {
  ...
  // 2. 收集需要安装的阶段（1..phase 累积）
  const allPhaseKeys = (
    Object.keys(PHASE_CONFIGS).map(Number) as Phase[]
  ).sort();
  const phases = allPhaseKeys
  ...
  const claudeMdPatched = await patchClaudeMd(config.target, config.dryRun);
