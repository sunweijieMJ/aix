...
- **数据驱动**：`v-model:nodes` / `v-model:edges` 双向绑定，数据结构即 Vue Flow 的 `Node` / `Edge`
- **内置节点与边**：圆形 / 六边形节点、可着色的 `ColorEdge`，业务字段放在 `data` 里
- **画布编辑**：双击空白新增节点、拖拽连线、选中删除、右键菜单，删除与连线都可拦截
- **多路径着色**：同一节点或边被多条路径共用时按颜色区分，非命中元素支持降透明与模糊
- **搜索定位**：内置搜索框按名称定位节点并居中视口
- **常驻名称**：节点名称以气泡常驻，支持按缩放阈值整体隐藏
- **样式隔离**：配色走 `--aix-flowGraph*` 组件级 CSS 变量，可整体换肤
