...
- **图标三种来源**：Vue 组件、图片地址、字体图标类名都可以直接写在 `icon` 上；分组标题的图标由 `showGroupIcon` 开启
...
- **细滚动条**：侧栏列表与 flyout 弹层统一 4px 圆角滑块，指针停在列表上才显示，颜色随主题
...
flyout 弹层里 `glass-dark` 沿用侧栏同款深色配色，其余三套共用白底配色。
