...
- 分组图标改为可配置，样式按设计稿校准
  ...
  - 分组标题图标默认不再渲染，由根组件 `showGroupIcon` 开启
  - 拖拽指示线改用 `#00c261`，把手命中区以侧栏右边缘居中，扩到 4px
  - 列表滚动条压在右侧留白上，菜单项宽度不随滚动条出现而变窄；滑块指针悬停时才显示
  - 搜索框清除按钮新增 `--aix-menu-search-clear-color` / `-hover` 两个变量，指针点击清空后回到默认态，键盘触发时焦点留在输入框
  - 选中项字重改为 500，字体图标不跟随加粗
  - `glass-dark` 的 flyout 弹层改用侧栏同款深色配色
