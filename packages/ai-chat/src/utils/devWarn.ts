 ...
 * 开发期护栏告警。
 *
 * 本包有一批「用法/配置错误」的护栏告警（未注册块渲染器、updateBlock 未命中、静态配置被
 * 运行时修改、triggers 触发字符重复……）。它们的受众是**集成本库的开发者**，在生产环境既帮不上
 * 忙，又会刷业务方的控制台——尤其 `markdownRenderers 为挂载时快照` 这类，业务方传内联对象时
 * 每次父级重渲染都可能误报。故统一收口到本函数，生产构建下静默。
 *
